...
## Commands Available
- `/develop` - **Planning-only with binding methodical-decomposition pre-work.** Phases 1a–1d gather context, study references, search mathlib, design the API. **Phase 1e (binding) is the pre-work pass**: for each top-level result, write the prose proof, decompose into ordered lemmas, **physically state every lemma in Lean as `:= by sorry` in the project files (Step 2.5 — the skeleton must `lake build` clean)**, then tension against the references with **a verbatim source quote per leaf plus a Lean ↔ source match paragraph in `decomposition.md` (Step 3)**. Per-leaf provability check: every leaf must be discharged by mathlib (cited + verified via `lean_loogle` / `lean_leansearch`) or already-developed project code (cited by file:decl), or recursively decomposed further. Leaves that cannot be discharged become explicit API gaps with their own sub-decomposition trees. Tickets (1g) only created after every leaf is verified across all three (declaration + quote + citation). Then 1h (ChatGPT validation if available) and 1i (user approval). Workers run via `/beastmode`. **Flag `--decompose`** runs ONLY Phase 1e and stops without creating tickets — for iterating on the decomposition / skeleton / source quotes before committing to a ticket board. Output summary includes skeleton build status, per-leaf source-check status, feasibility assessment.
