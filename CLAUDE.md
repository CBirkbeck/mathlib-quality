...
## Commands Available
- `/develop` - **Planning-only with binding methodical-decomposition pre-work.** Phases 1a–1d gather context, study references, search mathlib, design the API. **Phase 1e (NEW, binding) is the pre-work pass**: for each top-level result, write the prose proof, decompose into ordered lemmas, tension against the references, then run a per-leaf provability check — every leaf must be discharged by mathlib (cited, verified via `lean_loogle`/`lean_leansearch`) or by already-developed project code (cited by file:decl), or recursively decomposed further. Leaves that cannot be discharged become explicit API gaps with their own sub-decomposition trees. The artifact `.mathlib-quality/decomposition.md` is the proof the pass ran. Tickets (1g) are only created AFTER every leaf is verified — this prevents "we'll figure this step out during execution" from becoming a multi-week stop during `/beastmode`. Then 1h (ChatGPT validation if available) and 1i (user approval). Workers run via `/beastmode`.
