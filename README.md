...
| `/taupr` | **The Tau Ceti worker loop**, modelled on `TauCetiProject/TauCetiWorker`: a round does **one unit of work — the first that applies**. `REBASE` → `FIX-CI` (build red outranks findings — a PR that does not build cannot be reviewed) → `FIX` (fix the code, or contest **in the rubric thread the finding came from**) → `REVIEW` (**fallback only**: green build, unreviewed at the current head, and **≥1h since the build status**) → `AUTHOR` (**capped at 3 of our own PRs open**, else the round is `IDLE` — a building PR and a green PR inside its first hour both look like nothing to do, so an uncapped loop would author a new PR every tick). Maintenance outranks authoring so in-flight PRs are not starved; merging/closing/dedup belong to the repo's CI; and a GitHub API failure **aborts the round** rather than falling through to authoring duplicates. Authoring **checks the source material first** — roadmap as written, then review-quality library code, then the named source — so existing mathematics is adapted rather than rebuilt; then `/cleanup` on every changed file and `/decompose-proof` where needed; then push with `--force-with-lease` and open the PR. **CI reviews it** — we only self-review (`tauceti-review <PR> --reviewer codex --post`) a PR CI has left sitting for an hour. A pre-PR gate blocks `gh pr create` on the cheap checks only (cleanup coverage computed from the diff, source sweep, duplication) — no pre-PR review, so nothing waits on inference. One round per invocation — recurrence comes from the harness (`/loop 10m /taupr`, or a cron via the `schedule` skill), the same arrangement `/beastmode` uses; the interval is only the polling cadence and does not move the one-hour review threshold, which is measured from the `build` status. Also `--only`/`--skip` over the cascade, `status`, `--review-age`. Mechanics in `references/tauceti.md`. |
| `/pre-submit` | Pre-PR submission checklist (generic — on Tau Ceti use `/taupr`) |
...
**Never opening a PR that hasn't already passed review (the PR gate hook).** On a repo gated behind a scriptable reviewer, `/pre-submit` Step 8 runs the rubric on your **local branch** — a review engine taking `--diff-file` / `--pr-desc-file` / `--no-post` needs no PR to exist — and iterates until green. The observed failure mode is that a worker skips this and opens the PR to wait for the server reviewer instead: it feels like progress, so instruction text alone doesn't hold. The plugin therefore ships a `PreToolUse` hook (`hooks/pr_gate.sh`) that **blocks `gh pr create`** until Step 8 has written a green `.mathlib-quality/review-receipt.json` whose `head_sha` matches the current commit — stale receipts (branch moved since the review) block too, naming both commits.

**Tau Ceti specifics (`references/tauceti.md`).** Where the review is machine-run, the wire formats are not guessable and getting one wrong fails silently. Verified against the TauCeti/TauCetiReview repos: **a contest is a reply in the rubric thread the finding came from** (`POST /pulls/<PR>/comments/<ROOT_ID>/replies`) — a top-level PR comment is a different endpoint the engine never reads, and a body echoing `tauceti-reply:`/`tauceti-rubric:` is dropped as machine output, so quote the conflicting thread's prose and strip its markers. Contests don't re-trigger CI (the local worker owns them — run `tauceti-review <PR>` after). Review state comes from the newest scoreboard comment's `tauceti-meta` JSON, never the label. Pushes are `--force-with-lease` against the observed head. The roadmap gates *new* mathematics only — refactors need no roadmap entry. Duplication has a native mechanism (target markers + author claim refs) that beats any file-overlap heuristic.

**Not rederiving what a source repo already has (the source sweep).** Mathlib-absence had a hard standard — untruncated grep *and* a compiled probe — while source-repo absence had nothing but "checked ✓", so that's where duplicated work leaked in. `/pre-submit` Step 0b now demands a citable sweep per roadmap-named source: a **pinned clone** (`repo@sha` — a worker who never cloned can't produce one), the index read before grepping (blueprint, dep graph, `## Main results`), **three vocabularies** searched (yours, the source's convention, and the operator that can't be renamed away — that's the one that finds things), the *neighbourhood* read rather than the grep hits (results live as unnamed `have`s inside larger proofs), and a sub-lemma check. The gate rejects a sweep entry missing its revision or queries, and rejects an *empty* sweep when the chain's intake named a source. Method and the "what doesn't count" table in `references/pr-workflow.md`; `/beastmode` applies the same bar before declaring a MATHLIB GAP, since a gap already solved upstream is a port you haven't done.

**Not writing what's already in an open PR.** The same receipt carries a `duplication_check`, and the gate blocks a PR whose check is missing, stale, or records an unacknowledged overlap. The check runs twice: at Step 0b before code is written (a result can be absent from `main`, absent from mathlib, and still sitting in an open PR — most often your own earlier branch), and again at Step 8 before creating, because the open-PR list moves while you work. Overlap is ranked same-roadmap-target → same-declaration-names → same-files → same-source-section; a deliberate stacked follow-up is recorded `acknowledged: true` with a note rather than deleted. This matters precisely because the workflow keeps several branches in flight at once. Making the shortcut the blocked path means the cheapest route to a PR runs the dry run first. It is armed by `.mathlib-quality/pr-session.json` (so it's inert in any repo not running the workflow), fails open on infrastructure trouble, and escapes via `PR_GATE_OVERRIDE=1 gh pr create ...` or `touch .mathlib-quality/pr_gate_disabled`. Full workflow in `references/pr-workflow.md`.
