...
| `/taupr` | **The complete Tau Ceti PR pipeline**, branch to merge, including reviewing the PRs that get made. Discipline: **iterate to green privately, then open the PR, then immediately record the verdict.** **P6** reviews the *local branch* until every rubric is green — posting nothing, so the rounds it takes cost the PR nothing; this can't use `tauceti-review` (it takes a PR number and reads head/diff/description from GitHub), so it drives the inner engine `runner/review.py` with `--diff-file`/`--no-post`. **P8** then runs `tauceti-review <PR> --post` *immediately* after creating: a private run leaves no review state on GitHub and merging needs a GitHub-visible all-green review for the current head, so posting is what records it — and it's canonical (`newest_scoreboard`: "No access bar: any author") and claims the head so CI's metered run skips, letting your subscription displace the project's API bill. Around those: chain intake asked once, source sweep, duplication via the native claim-ref/target-marker mechanism, unconditional `/cleanup` + `/decompose-proof`, local gates, `--force-with-lease` push, contests **in the rubric thread the finding came from**, scoreboard monitoring while pipelining the next branch, auto-merge. Also `/taupr review [<PR>…] [--post]`, `/taupr contest`, `/taupr status`, `/taupr watch`. Mechanics verified against the TauCeti/TauCetiReview repos in `references/tauceti.md`. |
