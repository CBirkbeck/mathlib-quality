...
| `/taupr` | **The complete Tau Ceti PR pipeline**, branch to merge, including reviewing the PRs that get made. Because `tauceti-review` takes a **PR number** (no local-branch mode at the documented layer), the discipline is *open the PR, then immediately review it yourself and iterate to green* — on your own Claude/Codex subscription rather than CI's metered APIs. Eleven phases: chain intake asked once → source sweep + duplication via the native claim-ref/target-marker mechanism → unconditional `/cleanup` + `/decompose-proof` → local gates → PR body (`Roadmap:` line, target marker, provenance) → `--force-with-lease` push + create → `tauceti-review <PR>` (**dry run by default**, `--post` publishes as you) → contests **in the rubric thread the finding came from** → scoreboard monitoring while pipelining the next branch → auto-merge. Also `/taupr review [<PR>…] [--post]`, `/taupr contest`, `/taupr status`, `/taupr watch`. Mechanics verified against the TauCeti/TauCetiReview repos in `references/tauceti.md`. |
